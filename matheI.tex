		...
		\section{Primfaktorzerlegung (PFZ)}\index{Primfaktorzerlegung}\index{PFZ|see{Primfaktorzerlegung}}\index{PFZ}
			...
			\subsubsection{Satz 5.18 Hauptsatz der Arithmetik}\indexUnwichtig{Hauptsatz der Arithmetik}\label{Satz5.18}
				\begin{enumerate}[1)]
					...
					Falls
					\begin{align}
						&p_1 \geq p_2 \geq p_3 \geq \dots \geq p_r \text{ und}\\
						&q_1 \geq q_2 \geq q_3 \geq \dots \geq q_s
					\end{align}
					dann
					\begin{align}
						r &= s \text{ und}\\
	...
	\chapter{Gruppentheorie}
		...
		\section{Untergruppen}
			...
			\subsubsection{Definition 6.3}
				Sei $(G,\circlecdot)$ eine Gruppe, $H \subseteq G$. Dann heißt die kleinste Untergruppe $(\tilde{H},\circlecdot)$ von $G$, so dass $\tilde{H}\subseteq H$, die \subkeyword{von $H$ erzeugte Untergruppe}{Gruppen}.\indexUnwichtig{Untergruppen (Gruppen)!von $H$ erzeugte Untergruppe}
				\paragraph{Beispiele}
					\begin{itemize}
	...
	\section{Grundlagen und Isomorphismen}
		...
		\section{Matrizen}
			...
			\subsubsection{Definition 8.11}
				Sei $(K, +, \cdot)$ ein Körper, $m, n \inN$. Eine \emph{$(m \times n)$-Matrix mit Einträgen aus $K$}\index{Matrix mit Einträgen aus K@$(m \times n)$-Matrix mit Einträgen aus $K$} ist ein rechteckiges Schema
				\begin{align}
					\begin{bmatrix}
						a_{11}	& \cdots	& a_{1n}\\
						\vdots	& \ddots	& \vdots\\
				...
				Schreibweise: $(a_{ij})$ oder $(a_{ij})_{\scriptsize\begin{array}{c}1 \leq i \leq m \\ 1\leq j \leq n\end{array}}$\\
